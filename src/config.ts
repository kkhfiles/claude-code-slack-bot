...
export const config = {
  ...
  lunchBot: {
    ...
    // 소인의 주간 판단(`letter-initiative.ts` 를 같이 씀) — 주 첫 업무일 이 시각에 현황판·방의 지난주·맡은 일을 보고
    // (필요하면 웹 검색) 실장 DM 에 「이번 주 이렇게 할까요」. 방에 걸 글은 확인 카드로만(실장 2026-09-24).
    // 커피콩(13:00)과 겹치지 않게 30분 뒤. **기본은 꺼짐** — 커피콩과 같다.
    initiative: {
      enabled: process.env.LUNCH_INITIATIVE === '1',
      at: process.env.LUNCH_INITIATIVE_AT || '13:30',
    },
  },
  ...
  letter: {
    ...
    // 커피콩의 주간 시계(`letter-initiative.ts`) — 주 첫 업무일 이 시각에 현황판과 방의 지난
    // 7일을 보고 실장 DM 에 「이번 주 이렇게 할까요」. 방에 걸 글은 확인 카드로만 간다.
    // **기본은 꺼짐** — 만든 것과 켠 것은 다른 일이다. 빗장 일곱 겹은 그 파일 머리에.
    initiative: {
      enabled: process.env.LETTER_INITIATIVE === '1',
      at: process.env.LETTER_INITIATIVE_AT || '13:00',
    },
