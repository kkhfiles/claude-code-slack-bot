...
// 안 뜨는 말이라 그대로 사라졌다(2026-08-19 검토에서 잡음).
{
  ...
  // 좁은 길은 세션 자리를 대신하므로 **스스로 말해야 한다** — 안 받으면
  // 되물음이 통째로 사라지고 그 건은 닫힌다(실측 81건 중 여섯).
  eq('좁은 길의 되물음은 사람에게 간다',
     wa.boardOutputToTell('TSK-5 — 진행 로그' + String.fromCharCode(10) + '❓ 완료로 넘길까요?'),
     '❓ 완료로 넘길까요?');
  eq('좁은 길의 알림도 간다',
     wa.boardOutputToTell('TSK-5 — 진행 로그' + String.fromCharCode(10) + '💬 마감을 옮겼습니다'),
     '💬 마감을 옮겼습니다');
  // ⚠️ **대리 쌍이 쪼개지면 여기가 무너진다.** `💬` 는 BMP 밖이라
  // `u` 플래그 없는 문자 클래스에서 앞쪽 대리가 `🗂`(같은 앞쪽)와도 맞는다.
  eq('칸반 링크는 여전히 안 간다 (대리 쌍이 안 쪼개짐)',
     wa.boardOutputToTell('🗂 판 https://x/board/'), '');
}
